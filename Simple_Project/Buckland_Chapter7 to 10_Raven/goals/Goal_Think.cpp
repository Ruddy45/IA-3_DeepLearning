...
{
  if (notPresent(goal_attack_target) && (m_pOwner->GetNameTeam() == "" || IsGuideByTheLeader()))
  {
    RemoveAllSubgoals();
...
//----------------------- IsGuideByTheLeader ----------------------------------
//-----------------------------------------------------------------------------
bool Goal_Think::IsGuideByTheLeader()const
{
	return IsInATeam() && m_pOwner->GetTargetBot() != 0;
}
...
protected:
  ...
  // Nom de la team du bot pour reconnaitres ses confèreres, s'il en a une, sinon il est vide
...
public:
  ...
  // Le bot n'a de team que si on lui précise dans le constructeur ou dans la fonction SetTeam()
  ...
  // Définie le nom de la team et le leader lorsque le joueur choisie une cible lors d'une partie
  void			SetNameTeam(const std::string& newNameTeam, Raven_Bot* leader = 0) { m_sNameTeam = newNameTeam; m_pRavenBotLeader = leader; }

  // Définie les teamates du bot
  ...
  bool								 IsInATeam() { return GetNameTeam() != ""; } // Appartient à une team

...
protected:
  ...
  //the maximum speed this entity may rush target at.
  double      m_dMaxRushSpeed;
...
public:
  ...
  MovingEntity(Vector2D position,
	  ...
	  m_dMaxTurnRate(turn_rate),
	  m_dMaxForce(max_force),
	  m_dMaxRushSpeed(max_rush_speed)
  {
	  m_vPosition = position;
	  m_dBoundingRadius = radius;
	  m_vScale = scale;
  ...
  double    MaxRushSpeed()const { return m_dMaxRushSpeed; }
  void      SetMaxRushSpeed(double new_speed) { m_dMaxRushSpeed = new_speed; }

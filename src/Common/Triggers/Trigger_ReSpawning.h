...
public:
	...
	bool  m_bIsSpawning; // Si on doit spawn un objet ou non

  Trigger_Respawning(int id, bool isSpawning = true):Trigger<entity_type>(id),
                             m_iNumUpdatesBetweenRespawns(0),
                             m_iNumUpdatesRemainingUntilRespawn(0),
							 m_bIsSpawning(isSpawning)
  {}
  ...
  virtual void Update()
  {
    if (((--m_iNumUpdatesRemainingUntilRespawn <= 0) && !isActive() && m_bIsSpawning))
    {
      SetActive();
    }
  }

...
void Goal_AttackTarget::Activate()
{
  ...
  {
	/*for (std::map<int, Raven_Weapon*>::iterator it = m_pOwner->GetWeaponSys->m_Weapon.begin(); it != m_pOwner->GetWeaponSys->m_WeaponMap.end(); ++it)
	{
		if (it->second.GetType() == 11) {
			AddSubgoal(new Goal_RushTarget(m_pOwner));
		}
	}*/

...
#include "armory/Projectile_Knives.h"
...
{ 
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot)
  {
	...
	else if ((*curBot)->isAlive())
	{
		...
		/*
		//BOUCLE DE CREATION DU FICHIER DATA

		POINT p;
		bool save = false;
		//if ((m_TrainingSet.GetInputSet().size() < 505) && ((*curBot)->isPossessed())) {
		if ((m_TrainingSet.GetInputSet().size() < 10000) && ((*curBot)->isPossessed())) {
			//ajouter une observation au jeu d'entrainement
			//condition d'ajout au dataset
			if (int r = (rand() % 100) <= 1 || (*curBot)->isShooting()) {
				// on récupère l'endroit ou l'on vise et on modifie le target
				if (GetCursorPos(&p)) {
					if (ScreenToClient(GetActiveWindow(), &p)) {
						cursorPos = Vector2D(p.x, p.y);

						debug_con << p.x << " " << p.y << "";
						debug_con << "ID:" << (*curBot)->ID() << "";
						debug_con << (*curBot)->GetTargetBot() << "";
					}
				}
				// On ajoute au data
				vector<double> v1 = (*curBot)->GetDataShoot();
				vector<double> v2 = (*curBot)->GetTargetShoot();
				AddData(v1, v2);


				// On met shooting a false pour réinitialiser
				(*curBot)->setShootingFalse();
				debug_con << "la taille du training set" << m_TrainingSet.GetInputSet().size() << "";
			}
			if (m_TrainingSet.GetInputSet().size() == 10000) {
				if (save == false) {
					m_TrainingSet.WriteData();
					m_TrainingSet.SaveData();
					save = true;
				}
			}
		}
...
//Lancer l'apprentissage quand le jeu de données est suffisant
 //la fonction d'apprentissage s'effectue en parallèle : thread
	 ...
	 //AJOUT DES DONNEES AU TRAINING SET (UNE FOIS LE DATA AJOUTE)
	while (m_TrainingSet.GetInputSet().size() < 2500) {
		ifstream Echantillon("Echantillon.txt");
		if (Echantillon) {
			string ligne;
			int i = 0;
			while (i < 2500) {
				i++;
				getline(Echantillon, ligne);
				...
				vector<double> target = { (atof(cont.at(0).c_str())), (atof(cont.at(2).c_str())),(atof(cont.at(4).c_str())),(atof(cont.at(6).c_str())),(atof(cont.at(8).c_str())) };
				vector<double> input = { atof(cont.at(10).c_str()) };
				AddData(target, input);
				debug_con << "la taille du training set" << m_TrainingSet.GetInputSet().size() << "";

			}
		}
		else {
			debug_con << "Erreur de lecture de l'echantillon !" << "";
		}
	}

	if ((m_TrainingSet.GetInputSet().size() >= 2500) & (!m_LancerApprentissage)) {
...
void Raven_Game::ClickRightMouseButton(POINTS p)
{
  ...
  { 
	...
	CreateTeamBot("blue", 2); // Crée une équipe pour le leader
...
//-------------------------- CreateTeamBot ------------------------------------
//-----------------------------------------------------------------------------
void Raven_Game::CreateTeamBot(std::string nameTeam, int numberBotsInTeam)
{
	//clear any current goals
	m_pSelectedBot->GetBrain()->RemoveAllSubgoals();
	m_pSelectedBot->SetNameTeam(nameTeam, m_pSelectedBot); // Définie le nom de la team en bleu, le même que le leader
	m_pSelectedBot->TakePossession();

	// Crée une équipe pour le leader
	std::list<Raven_Bot*> team;
	for (std::list<Raven_Bot*>::iterator it = m_Bots.begin(); it != std::next(m_Bots.begin(), numberBotsInTeam); it++)
	{
		(*it)->SetNameTeam(m_pSelectedBot->GetNameTeam(), m_pSelectedBot); // Définie le nom de la team en bleu, le même que le leader
		(*it)->GetBrain()->RemoveAllSubgoals(); // Vide les bots de leurs buts
		(*it)->GetTargetSys()->SetTarget(0);// Vide les bots de leurs cible actuelles
		team.push_back(*it);
	}
	m_pSelectedBot->SetTeamates(team);
}

...
//-------------------------- ctor ---------------------------------------------
Raven_Bot::Raven_Bot(Raven_Game* world, Vector2D pos, std::string nameTeam):
  ...
  MovingEntity(pos,
               ...
               script->GetDouble("Bot_MaxForce")),
                 ...
                 m_Status(spawning),
                 m_bPossessed(false),
				 m_sNameTeam(nameTeam),  // Nom de la team du bot
                 m_dFieldOfView(DegsToRads(script->GetDouble("Bot_FOV"))),
				 m_pRavenBotLeader(0) // Leader
           
{
  // Spawn des armes possibles
  m_SpawnsTeamWeapon.push_back(new Spawn_TeamWeapon(Vector2D(30, 25), 7, 310));
  m_SpawnsTeamWeapon.push_back(new Spawn_TeamWeapon(Vector2D(440, 80), 7, 217));
  m_SpawnsTeamWeapon.push_back(new Spawn_TeamWeapon(Vector2D(30, 360), 7, 314));
...
void Raven_Bot::Update()
{
  ...
  {           
    ...
    if (m_pTargetSelectionRegulator->isReady() && !IsInATeam())
    {      
      m_pTargSys->Update();
    }
	...
	if (m_pTargSys->isTargetPresent()) {
	  ...
	  {
		  m_pTargSys->UpdatePossessed(m_pWorld->GetCursorPosition());
	  }
...
bool Raven_Bot::HandleMessage(const Telegram& msg)
{
  ...
  case Msg_TakeThatMF:
	...
	// Envoie le message au tireur pour dire qu'il a atteint sa cible
    ...
    if (isDead())
    {
		...
		if (m_pRavenBotLeader != 0 && m_pRavenBotLeader != this) // Envoie le message au leader qui va l'envoyer au reste du groupe, si ce n'est pa le leader
		{
			Spawn_TeamWeapon* randSpawn = m_SpawnsTeamWeapon[rand() % m_SpawnsTeamWeapon.size()]; // Spawn aléatoire
			m_pWorld->GetMap()->AddTeamWeapon_Giver(GetWeaponHighestAmmo(), GetNameTeam(), randSpawn->GetPosition(), randSpawn->GetRadius(), randSpawn->GetGraphIndex());
		}
    }
  ...
  case Msg_YouHitMe: // Indique à un leader qu'il a touché une cible
  {
	  if (m_bPossessed)
	  {
		  debug_con << "I hit an enemy of our nation " << ((Raven_Bot*)msg.ExtraInfo)->ID() << "";

		  // Envoie le message d'attaque à tout le groupe
		  for (std::list<Raven_Bot*>::iterator it = m_Teammates.begin(); it != m_Teammates.end(); it++)
		  {
				Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY,
					ID(),
					(*it)->ID(),
					Msg_TargetToAttack,
					(Raven_Bot*)msg.ExtraInfo);
		  }
	  }
  ...
  case Msg_TargetToAttack: // Donne la cible a attaqué selon le leader
  {
	  // Si la cible n'est pas le bot en lui même
	  if ((Raven_Bot*)msg.ExtraInfo != (Raven_Bot*)this)
	  {
		  debug_con << "We will destroy him " << ((Raven_Bot*)msg.ExtraInfo)->ID() << "";
  ...
  case Msg_EncircleEnemy: // Encercle la cible indiqué par le leader
  {
	  if (m_sNameTeam != "")
	  {
		  debug_con << "Encircle " << m_sNameTeam << "";
	  }
...
void Raven_Bot::Exorcise()
{
  ...
  // Remet le nom de la team des membres à zéro
  for (std::list<Raven_Bot*>::iterator it = m_Teammates.begin(); it != m_Teammates.end(); it++)
	  (*it)->SetNameTeam(""); // Remet à défaut le nom de la team

  m_Teammates.clear(); // Vide le tableau
...
//------------------------ GetWeaponHighestAmmo -------------------------------
//		Obtiens l'arme avec le plus de munition pour drop cette pour les 
//		autres membres de la team
//-----------------------------------------------------------------------------
double Raven_Bot::GetWeaponHighestAmmo()
{
	double ammo(0);
	double weapon(0);
	std::vector<int> type_Ammo = {type_rail_gun, type_shotgun, type_rocket_launcher, type_grenade_launcher, type_knife}; // Liste des armes

	// Vérifie qu'elle arme à le plus de munition
	for (int i = 0; i < type_Ammo.size(); i++)
	{
		if (ammo < GetWeaponSys()->GetAmmoRemainingForWeapon(type_Ammo[i]))
		{
			ammo = GetWeaponSys()->GetAmmoRemainingForWeapon(type_Ammo[i]);
			weapon = type_Ammo[i];
		}
	}
